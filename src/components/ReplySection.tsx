...
const ReplySection:React.FC<replyProps> = ({currTheme,singleMail}) => {
    ...
    return (
    <div  className='w-[800px] h-[600px] border border-gray-700 flex justify-between flex-col ' style={{borderColor: `${currTheme ? "#343A40" :  "#DFE3E8"}` }}>
        <div className=' relative w-[800px] flex justify-between flex-col ' >
            <div className='flex h-[70px] border py-3 pl-4' style={{borderColor: `${currTheme ? "#343A40" :  "#DFE3E8"}`}}>
                ...
                <div className='flex justify-end mr-6  gap-4 h-8 my-2 w-full text-right'>
                    <div className={`flex justify-center gap-2 p-2 items-center border border-gray-700 rounded ${currTheme? 'bg-[#222426]':'bg-[#fff]'}`} >
                        {/* <p className= 'w-3 h-3 rounded-3xl bg-yellow-600'></p> */}
                        <img src={yellow} alt="" />
                        <p className='text-[12px]'>Meeting Completed</p>
                        <ChevronDown/>
                    </div>
                   <div className={` flex justify-center p-2 items-center border border-gray-700 rounded ${currTheme? 'bg-[#222426]':'bg-[#fff]'}`} >
            ...
            <div className='py-3 pl-4 '>
                ...
                {
                    // <SingleMail currTheme={currTheme} subject={'New Product Launch'} fromEmail={'rajeshkumar'} toEmail={'rajeshkumar'} body={'heironer ggern oiergioe gerg oijgioejrg eijgegoje oije'}/>
                    singleMail?.length >0 && singleMail?.map((item:any)=>{
                        return <SingleMail currTheme={currTheme} {...item} key={item.id}/>
                    })
                }
